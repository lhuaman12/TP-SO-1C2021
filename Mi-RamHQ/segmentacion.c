...
int iniciarSegmentacion(void){
    ...
    //BITARRAY
    datos = asignarMemoriaBytes(TAM_MEM); //LLENA LOS CAMPOS EN 0
    ...
    int tamanio = bitsToBytes(TAM_MEM);
...
char* buscarTarea(int idPatota, int desplazamiento){ //EL DESPLAZAMIENTO ES LA PROX INSTRUCCION A EJECUTAR DEL TRIPULANTE

	char* tareas = buscarArrayTareas(idPatota);

    char* tarea = separarTareas(tareas, desplazamiento); //SEPARA LA TAREA Y ME DICE SI ES LA ULTIMA
...
t_segmento* elegirSegCriterio(t_list* segmentos, int size){
    ...
    log_info(logger,"Elijo segun %s", CRITERIO_SELECCION);

    if(strcmp(CRITERIO_SELECCION,"FF") == 0){//NO ENTRA LA CONCHA DE SU MADRE
...
void compactacion(){
    ...
    liberarBitMap(bitMapSegment, 0, TAM_MEM);
