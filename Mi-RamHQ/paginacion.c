...
int iniciarPaginacion(void){
    ...
    int tamanioPaginas = TAM_PAG; //ES IGUAL AL TAMANIO DE LOS FRAMES DE LA MEMORIA
    memoriaVirtualPath = PATH_SWAP;

    log_info(logger, "SWAP: %d",TAM_SWAP);

    cant_frames_ppal = TAM_MEM / tamanioPaginas;

    log_info(logger,"Tengo %d marcos de %d bytes en memoria principal",cant_frames_ppal, TAM_PAG);
    ...
    //MEMORIA VIRTUAL
    int tamanioSwap = TAM_SWAP;
    cant_frames_virtual = tamanioSwap / tamanioPaginas;

    log_info(logger,"Tengo %d marcos de %d bytes en memoria virtual",cant_frames_virtual, TAM_PAG);
...
t_list* guardarElementoPaginacion(void* algo, int tamanio){ //GUARDA EN PAGINAS NUEVAS -> VACIAS
    ...
    if(list_is_empty(framesSinOcupar)){ //NO HAY FRAMES LIBRES EN MEMORIA PPAL -> SWAP
        ...
        int cantPag = ceil((double) tamanio / (double) TAM_PAG);
...
void ocuparMemoriaPaginacion(void* algo, int tamanio, int idFrame, int desplazamiento, int mem){
    ...
    int base = ((idFrame * TAM_PAG) + desplazamiento);
...
t_list* completarEspacioEnPagina(void* estructura, t_pagina* paginaAOcupar, int size, int* desplazamiento){
    ...
    if(paginaAOcupar->tamanioDisponible >= size){       //ENTRA EN LA PAGINA JUSTO O LE SOBRA ESPACIO
        ...
        *desplazamiento = TAM_PAG - paginaAOcupar->tamanioDisponible;   //EMPIEZO A OCUPAR A PARTIR DE ESTO
...
void* sacarInfoPaginas(t_tabla_pagina* unaTabla){
    void* infoProceso = malloc(TAM_PAG*list_size(unaTabla->paginas));
...
void* copiarPaginasMP(t_list* paginas){
	...
	void* algo = malloc(TAM_PAG * list_size(paginas));
	...
	if(list_all_satisfy(paginas, (void*)enMP)){ //SI TODAS ESTAN EN MP, LAS COPIO
		...
		memcpy(algo, otroAlgo, TAM_PAG * list_size(paginas));
...
void* copiarPagina(t_pagina* pagina, int memoria){
    ...
    void* algo = malloc(TAM_PAG);

    // ME TENGO QUE FIJAR SI LO QUE TENGO QUE COPIAR ESTA EN MV O MP
    if(memoria == MEM_PPAL){

        int base = pagina->frame_ppal * TAM_PAG;
        pthread_mutex_lock(&mutexMemoria);
        memcpy(algo, memoriaPrincipal + base, TAM_PAG);
		pthread_mutex_unlock(&mutexMemoria);

    }else if(memoria ==MEM_VIRT){
        int base = pagina->frame_virtual * TAM_PAG;
...
void guardarMemoriaVirtual(t_pagina* pagina){
	...
	if(pagina->frame_virtual == -1){           //Si es -1 no esta en disco
		...
		ocuparMemoriaPaginacion(algo, TAM_PAG, frameEscogido->id, 0, MEM_VIRT);
		...
		ocuparMemoriaPaginacion(algo, TAM_PAG, pagina->frame_virtual, 0, MEM_VIRT); //Ocupo el mismo lugar que lo anterior con lo nuevo
...
t_list* buscarPaginas(int nPrimerPagina, int size, t_tabla_pagina* unaTabla, int relleno){
	...
	if(size <= relleno){   //relleno seria lo que le falta para completar la pagina, si lo que quiero es menor a este, solo uso una pag
		...
		t_list* otrasPag = buscarPaginas(nPagSig, size - relleno, unaTabla, TAM_PAG);
...
char* buscarTareasPaginacion(int idPatota){
    ...
    t_list* paginas = buscarPaginas(0, unaTabla->tamanioTareas, unaTabla, TAM_PAG);
...
t_tcb* buscarTripulantePaginacion(int idTripu, int idPatota){
	...
	int nPrimerPagina = base / TAM_PAG; //Con esto tengo en que pagina empieza

	int sobrante = ((nPrimerPagina+1) * TAM_PAG) - base;
	...
	if((base % TAM_PAG) == 0) //Si el resto es 0, estoy en el inicio de una pagina
	{
		unaTcb = copiarTcbPag(choclo, 0);
	}
	else//aca tengo la tcb desplazada dentro de la pag
	{
		int desplazamiento = base % TAM_PAG;
...
void sobreEscribirTcbPag(t_tcb* tcb, t_tabla_pagina* unaTabla){
	...
	if(list_all_satisfy(paginas, (void*)enMP)){
		...
		if((base % TAM_PAG) == 0){			//Si el resto es 0, estoy en el inicio de una pagina
			...
			desplazamiento = base % TAM_PAG;	//EL RESTO ME DA EL CORRIMIENTO DEL PTR DESDE EL INICIO DE LA PAGINA
		}
...
void actualizarElementoPag(void* algo, t_list* frames,  int size, int desplazamiento){
	...
	if(list_size(frames) == 1){ //USO UNA SOLA PAGINA
		...
		int relleno = TAM_PAG - desplazamiento; //RELLENO ES LO Q YA GUARDE
...
void p_expulsarTripulante(int idTripu){
	...
	int nPrimerPagina = base / TAM_PAG; //EN QUE PAGINA EMPIEZA

	int relleno = ((nPrimerPagina+1) * TAM_PAG) - base;	//RELLENO ES LO QUE OCUPA EL TRIPU EN LA PRIMERA PAGINA
...
void liberarPaginas(t_list* paginas, int size, t_tabla_pagina* unaTabla, int base, int relleno){
	...
	if(list_size(paginas) > 1){ //OCUPO MAS DE UNA PAGINA
		...
		if((base % TAM_PAG) == 0){	//SI ESTOY AL PRINCIPIO DE UN FRAME, LO LIBERO ENTERO
			...
			liberarPaginas(paginas, size - TAM_PAG, unaTabla, base + TAM_PAG,TAM_PAG);
			...
			list_remove(paginas,0);
			liberarPaginas(paginas, size - relleno, unaTabla, base + relleno, TAM_PAG);//Muevo la base el sobramte, que es el cachito que ocupa
		}
		...
		if(unicaPag->fragInterna + unicaPag->tamanioDisponible + size == TAM_PAG) {//SI LA SUMA DE TODO ESO DA IGUAL AL TAM DE UNA PAGINA, ENTONCES SIGNIFICA QUE NO HAY NADA MAS Y LA PUEDO BORRAR
...
void reemplazarPagina(){
	...
	if(string_equals_ignore_case(ALGORITMO_VM, "LRU")){
		reemplazarConLRU(paginasSinLock);
	}
	else if(string_equals_ignore_case(ALGORITMO_VM, "CLOCK")){
...
void revivirPagina(t_pagina* unaPagina){
	...
	t_list* lPagina = guardarElementoPaginacion(algo, TAM_PAG);//DEVUELVE UNA PAGINA, LA QUE GUARDASTE, SOLO NECESITO EL FRAME DONDE LO GUARDE
...
int calcularDirLogica(t_pagina* pagina, int desplazamiento){
    ...
    int dirLogica = (pagina->id * TAM_PAG) + desplazamiento;
    return dirLogica;
}
